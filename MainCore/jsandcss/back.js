...
async function uploadStrategy() {
  try {
    ...
    // Check if performanceMetrics data exists
    if ('performanceMetrics' in data) {
      ...
      performanceMetricsElement.innerHTML = `
        ...
        <p>Total Profit/Loss: ${metrics.totalProfitLoss}</p>
        <p>Win Rate: ${metrics.winRate.toFixed(2)}%</p>
      `;
    } else {
      console.error('Performance metrics data not found in server response.');
    }

    // Display buy and sell counts
    if ('buyCount' in data && 'sellCount' in data) {
      const buyCountElement = document.getElementById('buyCount');
      const sellCountElement = document.getElementById('sellCount');
      buyCountElement.textContent = `Buy Count: ${data.buyCount}`;
      sellCountElement.textContent = `Sell Count: ${data.sellCount}`;
    } else {
      console.error('Buy and sell counts not found in server response.');
    }
...
async function generatePlot() {
  try {
    ...
    const response = await fetch('http://127.0.0.1:5002/chart?stockName=' + stockName, {
      method: 'GET'
    });
